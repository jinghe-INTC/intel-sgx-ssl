...
# endif
    ...
    if (prov->init_function == NULL) {
        ...
        if (prov->module == NULL) {
            ...
            (void)(ERR_LIB_CRYPTO, ERR_R_DSO_LIB,
                           "name=%s", prov->name);
            goto end;
        }
    ...
    if (prov->init_function == NULL) {
        (void)(ERR_LIB_CRYPTO, ERR_R_UNSUPPORTED,
                       "name=%s, provider has no provider init function",
                       prov->name);
        goto end;
    }
    int prov_init_result = prov->init_function((OSSL_CORE_HANDLE *)prov, core_dispatch,
                             &provider_dispatch, &tmp_provctx);
    if (!prov_init_result) {
        (void)(ERR_LIB_CRYPTO, ERR_R_INIT_FAIL,
                       "name=%s", prov->name);
        goto end;
    }
