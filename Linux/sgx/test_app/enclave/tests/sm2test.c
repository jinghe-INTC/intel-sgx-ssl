...
static int start_fake_rand(const char *hex_bytes)
{
    ...
    if (NULL == (saved_rand = RAND_get_rand_method()))
        return 0;
                          ...
                          const char *k_hex, const char *ctext_hex)
{
    ...
    if (!(sm2_encrypt(key, digest, (const uint8_t *)message, msg_len,
                               ctext, &ctext_len))) {
        restore_rand();
        goto done;
    }
...
int sm2_test(void)
{
    ...
    printf("SM2 is disabled.");
#else
    if ( 0 == sm2_crypt_test()) ret++;
    if ( 0 == sm2_sig_test()) ret++;
#endif
    return ret;
}
